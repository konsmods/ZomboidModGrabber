...
- Everything is saved to JSON files: the default server list is
  `data/mods.json`, and additional presets are stored as
  `data/presets/<preset>.json`. Use the Preset selector in the UI to switch
  between different server mod collections.
- The web UI is just a thin client over a small JSON API (`/api/presets`,
  `/api/mods`, `/api/scan`, `/api/reorder`, `/api/reorder-collections`,
...
2. Select an existing server preset or click **New preset** to create a
   separate mod list for another server.
3. New mods appear at the bottom of their collection. Drag the ⠿ handle to
...
4. If a mod shows "no Mod ID found" or "N mod ids — verify", open its
   Workshop page, check the description, and edit the Mod ID field directly
   in the list (it saves on blur/Enter).
5. Toggle **Build 42 format** if your server is on B42+ (prefixes each Mod
   ID with `\`, per PZ's current config format). Turn it off for Build 41.
6. Hit **Copy** next to `Mods=` and `WorkshopItems=` and paste both lines
   into your server's `.ini`. A `Map=` line appears too if any scanned mod
   declares a map folder.
7. Re-run a scan on the same collection any time you add mods to it on
...
- Preset JSON files are per-machine local storage; nothing is sent anywhere
